...
const addSubAccount = asyncHandler(async (req, res) => {
    ...
    if (!businessName || !bankCode || !accountNumber) {
        throw ApiError.badRequest('Business name, bank code, and account number are required');
    }
    ...
    if (user.organizerProfile?.paystack?.isActive) {
        throw ApiError.badRequest('Paystack subaccount already exists. Contact support to update.');
    }
    ...
    if (!result.success) {
        throw ApiError.badRequest(result.message || 'Failed to create Paystack subaccount');
    }
