...
const createOrganizerSubaccount = async (req, res) => {
  try {
    const { bankCode, accountNumber, businessName, percentageCharge } =
      req.body;
    const user = await User.findById(req.params.id);
    ...
    if (user.organizerProfile?.platformStatus !== "approved") {
      return res
        .status(400)
        .json({ message: "Organizer must be approved first" });
    }
    ...
    const result = await paystackService.createSubaccount(user, {
      ...
      accountNumber,
      businessName:
        businessName || user.organizerProfile.businessName || user.fullName,
